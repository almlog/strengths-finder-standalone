...
### v3.5.0: 勤怠分析 ビジュアル強化 (2026-01-14)

#### ✨ 新機能: サマリータブにグラフ追加

ストレングスファインダー分析と統一されたUXを提供し、データを視覚的に理解できるようにしました。
...
**違反種別分布チャート（円グラフ）**
- 違反種別ごとの分布をドーナツチャートで表示
- パーセンテージ表示付きラベル
- 違反がある場合のみ表示

**36協定アラート分布チャート（横棒グラフ）**
- 従業員のアラートレベル別人数を視覚化
- 正常 → 注意 → 警告 → ... → 法令違反の順で表示
- 8段階カラーで深刻度を表現
...
### v3.4.3: 個人PDF 申請サマリー拡張 (2026-01-14)
...
- 振替出勤、振替休日、休日出勤、代休
- 欠勤、特休、慶弔
- 生理休暇、子の看護休暇、介護休暇、明け休

**就業実績時間セクション追加**
- 勤務サマリーと残業・36協定の間に新セクション追加
- 月間総就業時間を青色背景で大きく表示

#### 🐛 バグ修正

**違反詳細のテキスト切れ修正**
- 問題: 違反詳細の説明が `truncate` クラスにより途中で切れていた
- 修正: truncateを削除し、日付・違反種別を固定幅に設定
...
### v3.4.2: 勤怠分析 PDF出力 & サマリー詳細表示 (2026-01-11)

#### ✨ 新機能: PDF出力

**概要**
- 勤怠分析サマリーをPDFでエクスポート可能に
- 役員・上長・総務が1枚で状況把握できるレイアウト

**PDF出力内容**
- 分析対象期間、対象部署、対象人数
- 分析実行日時
- 全体統計（残業時間、休日出勤、違反サマリー）
- 36協定アラート、予兆アラート

**技術詳細**
- `html2canvas` + `jspdf` を使用
- A4横向きで1ページに収まるようスケーリング

#### ✨ 新機能: サマリーカード詳細表示

**概要**
- 「問題あり」「高緊急度」「中緊急度」カードをクリックすると該当者一覧をモーダル表示
- どのメンバーが該当しているか一目でわかるように
...
**表現の改善**
- 「問題あり従業員」→「確認が必要なメンバー」
- 「高緊急度従業員」→「法令違反の可能性がある対象者」
- 「中緊急度従業員」→「届出漏れがある対象者」
- より柔らかく、対応を促すニュアンスに変更
...
### v3.4.1: 勤怠分析 UX改善 (2026-01-11)

#### ✨ 新機能: 「今日を含める」トグルスイッチ

**背景**
- 業務時間中に勤怠分析を行うと、当日は退勤打刻前のため「勤怠未入力」として誤検出される問題
- ユーザーからの要望: 当日を分析対象から除外するオプションが欲しい

**実装内容**
- ファイルアップロード前に「分析オプション」セクションを追加
- 「今日の日付を含める」トグルスイッチを実装
- トグル状態に応じてラベルと説明が動的に変化:
  - OFF: 「今日を除外して分析」「退勤前の誤検出を防止」
  - ON: 「今日を含めて分析」「退勤前でも違反判定される場合あり」

**技術詳細**
- `src/models/AttendanceTypes.ts`: `AnalysisOptions`インターフェース追加
- `src/services/AttendanceService.ts`: `analyzeExtended()`、`createEmployeeMonthlySummary()`がオプションを受け取るよう拡張
- `src/components/attendance/AttendanceAnalysisPage.tsx`: トグルUI実装、状態管理

#### 🎨 UI/UX改善

**アップロードエリアのレイアウト変更**
- 注意事項バナーと分析オプションを分離して明確化
- 「分析オプション」セクションを追加（背景色で視覚的に区別）
- トグルスイッチを大きめに表示（11x6サイズ）
- 現在の状態が一目で分かるラベル設計

**従業員別タブの役職バッジ**
- 各従業員の役職を色付きバッジで表示
- 部長/マネージャー/リーダー/主任などを視覚的に識別可能

#### 🐛 バグ修正

**残業時間の取得元修正**
- 問題: 残業時間が週40超過カラムから取得されていた
- 修正: 36協定カラム（インデックス58）から正しく取得

**Excel日付形式の解析修正**
- 問題: XLSXライブラリの`raw:false`オプションにより日付が"M/D/YY"形式で返される
- 修正: `parseDate()`と`parseDateTime()`に"M/D/YY"パターンを追加
...
**動的ヘッダー表示**
- 部署分析: "部署コード: DEPT_B01 の代表的な強み"（選択中の部署コードを表示）
