...
- ビルド成果物内の日本語テキストはUnicodeエスケープされる（`\u30B5\u30DE\u30EA\u30FC`等）。Grepで直接ヒットしないため、英数字キーワード（バージョン番号、CSS クラス名等）で検証する
- 「ビルド成功」と「成果物に変更が含まれている」は別。ビルド後は必ず成果物の中身を検証する
- ローカルビルド完了 ≠ 本番反映。commit → push → CI/CD成功 → 本番反映の全ステップを追跡し報告する
- **CIワークフロー（deploy.yml）は `npm test` を実行しない**（ビルド＋デプロイのみ）。CI成功はテスト成功を意味しないため、push前のローカル全テスト実行が唯一の砦（2026-07-31発見。mainに既存失敗91件が蓄積していた）
- セッション開始時は必ず `git fetch` してorigin/mainとの乖離を確認する。ローカルが古いまま「機能が存在しない」と誤認して重複実装した事故あり（2026-07-31）
- **CRAのjest設定は `resetMocks: true`**。`jest.fn(impl)` でモジュールスコープに定義した実装は各テスト前に消える。モックの実装は素のアロー関数に持たせ、`jest.fn()` は呼び出し記録のみに使う（2026-08-26発見、LineWorksService.test.tsが正しいパターン）
- **外部サービス依存は定期的に死ぬ**: 無料CORSプロキシ allorigins.win 完全ダウン、JR東日本RSS配信終了（403/404）、Yahoo!路線情報ページ刷新でHTML正規表現パース全滅（実体は `__NEXT_DATA__` JSONへ）。外部取得は必ず「取得失敗」と「データなし」を区別して可視化する（2026-08-26）
- Bashツールのcwdは呼び出し間で持続する。`cd functions && ...` の後は必ずルートに戻さないと後続の `npm test` が Missing script で失敗する（2026-08-26）
