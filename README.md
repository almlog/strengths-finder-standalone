...
- 📊 **勤怠分析（v3.4）**: 楽楽勤怠連携による労務管理支援
  ...
  - 出社2/出社3のギャップを休憩として補正（午前有休時の休憩不足誤検出を解消）
  - StrengthsFinder連携（従業員のTop5表示）
- 🎙️ **スタラジ（v3.9）**: 社内Podcastの配信一覧・再生・セグメント表示
  - エピソード一覧（NEWバッジ・過去分折りたたみ・カレンダービュー）
  - HTML5 Audio再生コントロール（play/pause/seek/mute）
  - セグメントクリックでシーク再生
  - キャスト情報の色付きチップ表示
  - GitHub Pages配信フィード + localStorageキャッシュ（1時間）
...
- **🆕 勤怠分析（v3.4）**: 楽楽勤怠からエクスポートしたXLSXを分析
  ...
  - **🆕 休憩時間のギャップ補正（v3.9.2）**: 退社→出社2（退社2→出社3）の間の空き時間を休憩として自動集計。午前有休などで楽楽勤怠が休憩をカウントしない日でも、出社2/退社2の打刻で取った休憩が正しく認められ、休憩不足の誤検出がなくなります。補正後も休憩が足りない場合は従来どおり違反として検出します
  ...
  - **🆕 シート名からの始業時刻抽出（v3.4.4）**: 早出判定の精度向上
    - シート名パターン（例: `CLIENT_A_日勤_800-1630～930-1800`）から始業時刻を自動抽出
...
社内Podcast「スタラジ」の配信一覧・再生機能です。

#### 🎙️ 機能
- **エピソード一覧**: 最新エピソードをNEWバッジ付きで強調、過去分は折りたたみ
- **カレンダービュー**: 月ナビ + 配信日ドット表示で配信スケジュールを俯瞰
- **再生コントロール**: play/pause/seek/mute対応のHTML5 Audioプレーヤー
- **セグメント表示**: タイムスタンプ付きのトピック一覧、クリックでシーク再生
- **キャスト情報**: MC・アシスタントを色付きチップで表示

#### 🔧 技術
- GitHub Pages (`almlog.github.io/starradio-feed`) からエピソード情報をfetch
- localStorageに1時間キャッシュ（fetch-on-demand方式）
- レスポンシブレイアウト（PC: 横並び / モバイル: 縦積み）
