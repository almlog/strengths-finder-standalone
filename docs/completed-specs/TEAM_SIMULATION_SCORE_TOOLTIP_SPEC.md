...
70以上: リーダー型（高い統率力）
  → 明確な指示系統と迅速な意思決定
  → 向いているプロジェクト: 緊急対応、大規模な組織改革
50-70: バランス型（あなたのチーム ✓）
  → リーダーシップの分散と柔軟な役割分担
  → 向いているプロジェクト: 通常のプロジェクト、適度な変化
50未満: 専門家型
  ✨ ユニーク: フラットで民主的なチーム
  → 全員が専門家としての意見を持つ
  → ボトムアップの意思決定
  → 創造的な議論と合意形成
  → 向いているプロジェクト: 研究開発、創造的な探索

【スコアを上げるには】
• E型（外向的）メンバーを追加: +15点/人
  例: ENTJ、ESTJ、ENFJなどの性格タイプ
• T型（論理思考）メンバーを追加: +12点/人
  例: INTJ、ISTJ、ENTJなどの性格タイプ
• J型（計画的）メンバーを追加: +18点/人
  例: ISTJ、ESTJ、INFJなどの性格タイプ

【重要】高スコア = 正解ではありません
あなたのプロジェクトに最適なスコアを選びましょう！
```
...
**既存デザインを踏襲**（Rechartsのツールチップと統一）

```tsx
// Tailwindスタイル - 既存の白背景デザイン
<div className="
...
**重要**:
- `cursor: help` は使用しない（?マークを出さない）
- 通常のポインターで、ホバー時のみツールチップ表示
...
### Phase 2: 改善提案（推奨）
- [x] スコア向上のための具体的な提案生成（「スコアを変えるには」として実装）
- [ ] 「誰を追加すればいいか」のレコメンデーション（今後の拡張）
...
### 受け入れテスト
- [x] 各スコアのツールチップが正しく表示される
- [x] 計算式の内訳が正確である（個人スコア平均方式に修正済み）
- [x] 改善提案が実行可能である（「スコアを変えるには」として実装）
- [x] モバイルでも見やすい（レスポンシブ）
- [x] ダークモード対応（白背景デザインで対応）
