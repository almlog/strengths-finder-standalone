...
### v3.5.1 - 勤怠分析修正 + 駅データ改善（2026年1月）

**リリース日:** 2026-01-29

**主要機能:**
- **深夜休憩違反検出ロジックの修正**: 誤検出を防止する閾値ベースの新ロジック
  - 深夜労働30分以上 + 休憩時間不足の両条件を満たす場合のみ違反検出
  - 22:15退勤（深夜15分）のような軽微なケースでの誤検出を防止
- **駅データの静的埋め込み**: ODPT API依存からの脱却
  - 約90駅、15路線のデータを静的に埋め込み
  - APIが利用できない場合も駅選択が可能（手動入力でカバー）

**既知の制限:**
- 駅データのマッチング問題が一部残存（手動入力機能でカバー）
...
**主要機能:**
- Mini Tokyo 3D統合によるリアルタイム運行情報
- 遅延情報ティッカー（マーキー表示、5分自動更新）
- 遅延履歴モーダル（6時間分の履歴表示）
- 遅延報告メッセージ作成機能
- 電車タップ連携（路線名自動入力）
...
| バージョン | リリース日 | 主要機能 |
|-----------|-----------|---------|
| v3.5.1 | 2026-01-29 | 深夜休憩違反検出修正、駅データ静的埋め込み |
| v3.5 | 2026-01-28 | 交通情報タブ、遅延ティッカー、遅延報告作成 |
...
#### v3.5.1: 勤怠分析修正 + 駅データ改善（2026-01-29）

**バグ修正:**
- 深夜休憩違反検出の誤検出を修正
  - 変更前: 深夜労働があれば即座に違反検出
  - 変更後: 深夜労働30分以上 + 休憩時間不足の両条件を満たす場合のみ
- 22:15退勤（深夜15分）のような軽微なケースでの誤検出を防止

**改善:**
- 駅データの静的埋め込み（約90駅、15路線）
- ODPT API依存からの脱却
- APIが利用できない場合も駅選択が可能

**テスト:**
- AttendanceServiceの191テストが全PASS

---

#### v3.5: 交通情報タブ（2026-01-28）
...
| ログファイル | 内容 |
|-------------|------|
| [Log_2026-01-29_1000_Attendance_NightBreak_Station_Data.md](../dev_logs/Log_2026-01-29_1000_Attendance_NightBreak_Station_Data.md) | 深夜休憩違反修正、駅データ静的埋め込み |
| [Log_2026-01-28_1000_Delay_Ticker_Feature.md](../dev_logs/Log_2026-01-28_1000_Delay_Ticker_Feature.md) | 遅延情報ティッカー機能 |
