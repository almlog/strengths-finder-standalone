...
- **スタラジ復帰告知（v3.9.5）は本番適用済み**
  - ラズパイ修理完了。「おまたせしました！復帰しました！」バナーを2026-09-16まで表示（期限後は自動で通常表示へ）
  - 新しい回が実際に配信されているか、次セッションでepisodes.jsonの最新日付を確認するとよい
- **スタラジ配信停止バナー（v3.9.4）は本番適用済み**
  - ラズパイNW不調で新しい回が2026-08-27から止まっている。最新回が2日より古ければ「確認中」バナーを自動表示（復旧で自動消滅、D-005）
  - 過去配信分はGitHub Pages上のため再生可能。ラズパイ復旧後は新しい回がpushされることだけ確認すればよい
