...
2. **遅延情報ステップ③: 取得失敗の可視化**
   - fetch失敗時に「平常運転」（緑）ではなく「取得失敗」を表示する
   - `DelayTicker.tsx` は `delays.length === 0` で緑表示になるため、失敗と平常の区別が必要
   - `TrainDelayService.fetchDelayInfo` はエラー時にキャッシュを `[]` で上書きする現行仕様（テストに文書化済み）も併せて見直す
3. **テスト失敗86件（19スイート）の調査**（DASHBOARD 残タスク優先度:高、91件から改善済み）
