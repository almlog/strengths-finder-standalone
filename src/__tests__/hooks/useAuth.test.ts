...
describe('useAuth', () => {
  ...
  describe('ローカル開発環境での管理者判定', () => {
    it('Emulator環境で LOCAL_ADMIN_EMAILS に含まれるメールは管理者になる', async () => {
      ...
      const mockUser = {
        uid: 'local-admin-uid',
        email: 'suzuki.shunpei@example.com', // LOCAL_ADMIN_EMAILS に含まれる
        getIdTokenResult: jest.fn().mockResolvedValue({
          claims: {}, // roleクレームなし
        }),
      };
    ...
    it('Emulator環境でも LOCAL_ADMIN_EMAILS に含まれないメールは一般ユーザー', async () => {
      ...
      const mockUser = {
        uid: 'local-user-uid',
        email: 'other@example.com', // LOCAL_ADMIN_EMAILS に含まれない
        getIdTokenResult: jest.fn().mockResolvedValue({
          claims: {},
        }),
      };
    ...
    it('本番環境では LOCAL_ADMIN_EMAILS は無視される', async () => {
      ...
      const mockUser = {
        uid: 'prod-uid',
        email: 'suzuki.shunpei@example.com', // LOCAL_ADMIN_EMAILS に含まれるが無視される
        getIdTokenResult: jest.fn().mockResolvedValue({
          claims: {}, // roleクレームなし
        }),
      };
